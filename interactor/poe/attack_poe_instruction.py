...
def clear_chat():
    ...
    button = wait.until(EC.element_to_be_clickable((By.XPATH,
                                                    '/html/body/div[1]/div/div[1]/div/main/div/div/div[1]/div/div/footer/div/div[2]/div/div/div/div[2]/div[1]/button')))
    button.click()
...
def input2LLM(attack_prompt):
    ...
    time.sleep(
        30)  # div.ChatMessagesView_tupleGroupContainer__LSCLm > div:last-child > div:nth-child(2) > div.LeftSideMessageHeader_leftSideMessageHeader__5CfdD > div.DropdownMenuButton_wrapper__uc04T.ChatMessageOverflowButton_overflowButtonWrapper__gzb2s > button > svg
    more_button = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR,
                                                             "div.ChatMessagesView_tupleGroupContainer__LSCLm > div:last-child > div:nth-child(2) > div.LeftSideMessageHeader_leftSideMessageHeader__5CfdD > div.DropdownMenuButton_wrapper__uc04T.ChatMessageOverflowButton_overflowButtonWrapper__gzb2s > button")))
    driver.execute_script("arguments[0].click();",
                          more_button)  # /html/body/div[1]/div/div[1]/div/main/div/div/div[1]/div/div/div[1]/div/div/div[12]/div[2]/div[1]/div[2]/button/svg
    time.sleep(1)
    copy_button = wait.until(EC.element_to_be_clickable((By.XPATH,
                                                         "//button[contains(@class, 'DropdownMenuItem_item__EIRcp') and (.//text()[contains(., '复制信息')] or .//text()[contains(., 'Copy message')])]")))
    copy_button.click()
    # copy
...
def LLMThief(attack_prompts, url, level, ground_truth=None):
    ...
    if level >= 2:
        leaked_instruction, score, scorelist, bestssresponse, bestsscore, sslist, fail = steal_instruction(
            attack_prompts, level, ground_truth)
        return leaked_instruction, score, scorelist, bestssresponse, bestsscore, sslist, fail
    elif level == 1:
        leaked_instruction, score, scorelist, bestssresponse, bestsscore, sslist = steal_instruction(attack_prompts,
                                                                                                     level,
                                                                                                     ground_truth)
